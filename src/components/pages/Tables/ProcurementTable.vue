    ...
    methods: {
      _handleExport(val) {
        ...
        import ('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['订单编号', '药品编号', '品名', '供应商名','进价(￥)','价格(￥)','采购数量','采购金额','采购时间','状态']
          const filterVal = ['stockNo','goodsNo','goodsName','supName','stockPrice','goodsPrice','stockCount','stockMoney','stockAddTime','stockState']
          const list = val
          const data = this.formatJson(filterVal, list)//格式化时间
          excel.export_json_to_excel(tHeader, data, this.fileName)//导excel文件
          this.loading = false
        })
      },
      handleExportAll() {
