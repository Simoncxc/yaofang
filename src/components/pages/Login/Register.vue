...
<script>
  ...
  export default {
    ...
    methods: {
      submitForm(formName) {
        let admName = this.regForm.userName//将表单赋值
        let admPassword = this.regForm.password
        let admAddTime = moment().format('YYYY-MM-DD HH:mm:ss')//生成注册时间
        let admEditTime = admAddTime
        let role = this.regForm.role
        this.$refs[formName].validate((valid) => {
          if (valid) { //验证通过
            axios.post('api/admin/register', {//赋值返回后端
                admName,
                admPassword,
                admAddTime,
                admEditTime,
