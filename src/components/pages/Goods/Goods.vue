<template>
  <div class="goods">
    ...
    <el-form :inline="true" :model="filters">
      ...
      <el-form-item>
        <el-input v-model="filters.goodsNo" clearable @keydown.enter.native="handleKey" placeholder="请输入编号"></el-input>
      </el-form-item>
      <el-form-item>
        <el-input v-model="filters.goodsType" clearable @keydown.enter.native="handleKey" placeholder="请输入分类"></el-input>
      </el-form-item>
      <el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleMultiDel(val) {
        if (this.multipleSelection.length == 0) {//判断是否选择
          this.$message.warning('请选择')
        } else {let multi = this.multipleSelection
          this.$confirm('你确定？', '提示', {}).then(() => {

              let goodsNo = multi.map(el => el.goodsNo)//生成药品编号数组
              axios.post('api/goods/multiDel', {//提交数据到后端
                  goodsNo: goodsNo
                })
                .then(res => {
      ...
      addSubmit() {
        this.$refs.addForm.validate(valid => {//验证表单数据
          if (valid) {
            let params = Object.assign({}, this.addForm);//聚合数据为对象
            let goodsAddTime = moment().format('YYYY-MM-DD HH:mm')
            axios.post('api/goods/add', {//提交数据到后端
              goodsName: params.goodsName,
                goodsType: params.goodsType,
                goodsUnit: params.goodsUnit,
                goodsModel: params.goodsModel,
                ...
                this.addVisible = false//关闭面板
                this.queryAll()//获取所有药品
                let page = Math.ceil(this.page / 10) + 1//计算页数
                this.handleCurrentChange(page)
              })
              .catch(err => console.log(err))
          }
