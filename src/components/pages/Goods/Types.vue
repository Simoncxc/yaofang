...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleAdd() {//新增分类
        let typeAddTime = moment().format('YYYY-MM-DD HH:mm')//生成时间
        axios.post('api/types/add', {//提交后端
            goodsType: this.addType,
            typeAddTime: typeAddTime
          }).then(res => {
            this.$message.success('添加成功')
            this.addType=''
            this.queryAll()//刷新分类列表
          })
          .catch(err => console.log(err))
      },
      handleEdit(row) {
      ...
      handleDel(row) { console.log(row)
        this.$confirm('删除该记录，确定？', '提示', {
            ...
            axios.post('api/types/del', {//提交后端
                goodsTypeNo: row.goodsTypeNo
              }).then(res => {
                let index = this.types.indexOf(row);//计算当前行
                this.types.splice(index, 1);//删除当前行数据
                this.$message.success('删除成功')
                this.loading = true
                this.queryAll()//刷新列表
              })
