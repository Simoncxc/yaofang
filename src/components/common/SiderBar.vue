...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      addMenu() {
        const token = this.$store.state.user.token//判断是否存在token
        if (token) {
          const decoded = checkToken(token, 'cysto');//通过token判断用户角色
          this.items = decoded.role === 'admin' ? admItems : editorItems//通过不同角色加载不同界面
        } else {
          return false
        }
      }
