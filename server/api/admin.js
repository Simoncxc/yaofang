...
module.exports = {
  login(req, res) {
    ...
    conn.query(sql, [params.admName], function (err, result) {//查询是否存在该用户
      ...
      if (!result.length) {
        ...
        bcrypt.compare(admPass, hashPass, (err, sure) => {//验证密码
          if (sure) {
            res.json({
              code: 200,
              msg: '登录成功',
  ...
  register(req, res) {
    ...
    let params = req.body;
    const token = createToken(params.admName, params.role)//创建token
    bcrypt.hash(params.admPassword, 10, (err, hash) => {//哈希加密密码
      if (err) console.log(err)
      params.admPassword = hash
      conn.query(sql, [//判断数据库是否写入注册数据
        params.admName,
        params.admPassword,
        params.admAddTime,
        params.admEditTime,
